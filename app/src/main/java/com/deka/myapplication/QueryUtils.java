...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes(String requestUrl) {
        ...
        URL convertedUrl = createURL(requestUrl);
        HttpsURLConnection httpsURLConnection = null;
        String jsonResponse = "";
        InputStream inputStream = null;

        try {
            httpsURLConnection = (HttpsURLConnection) convertedUrl.openConnection();
        ...
        try {
            ...
            long time;
            String url = null;
            JSONObject sample = new JSONObject(jsonResponse);
            JSONArray featuresArray = sample.optJSONArray("features");
            for(int i=0;i<featuresArray.length();i++){
                JSONObject properties = featuresArray.getJSONObject(i).optJSONObject("properties");
                mag = properties.optString("mag");
                place = properties.getString("place");
                time = properties.getLong("time");
                url = properties.getString("url");
                earthquakes.add(new Earthquake(Double.parseDouble(mag),place,time,url));
